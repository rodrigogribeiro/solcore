...
import Solcore.Primitives.Primitives (primVals, primTypes, primClasses, eqInstances, refInstances)
-- basic type synonyms 
...
type Specialisation = (Name, Ty, [Arg], Expr)
type ResolutionEnv = Map.Map Name [Resolution]
type Resolution = (Ty, Expr)
...
initState = init TcState
  ...
  , tcsCoverageEnabled = True
    ...
    primInsts = eqInstances ++ refInstances

    init :: TcState -> TcState
    init = addInstances primInsts . addPrimClasses
...
withCurrentSubst :: HasType t => t -> TCM t
...
showEnv withPrims env = concatMap showEntry $ Map.toList env where
    showEntry (n, s)
        | not (nameIsPrimitive n) || withPrims = unName n ++ " : " ++ str (legibleScheme s) ++ "\n"
        | otherwise = ""
...
showREnv = unlines . map showRes . Map.toList where
    ...
    showRes (n, res) = intercalate "\n"  (map showEntry res) where
      ...
      showEntry (t, e) = unName n ++ " : " ++ str t ++ " = " ++ showExpr e

nameIsPrimitive n = isJust (Map.lookup n primEnv)

showSmallEnv :: [(Name,Scheme)] -> String
showSmallEnv env = intercalate ", " [showEntry (unName n,s) | (n,s) <- env, not (nameIsPrimitive n)]
    where
    withPrims = False
...
askType n = do
  ...
  case result of
    Just t -> return t
    Nothing -> throwError $ "Unknown name: " ++ (unName n)
...
tcmDeplete :: TCM Tyvar
tcmDeplete = (TVar . Name) <$> lift nssDeplete
...
legibleScheme (Forall tvs ty) = Forall tvs1 ty' where
    tvs' = Name <$> take (length tvs) namePool
    tvs1 = map TVar tvs'
    subst = Subst $ zip tvs (map TyVar tvs1)
    ty' = apply subst ty
...
renameFreeVars :: HasType a => a -> TCM a
...
newTVar :: Tyvar -> TCM (Tyvar, Ty)
newTVar tv = do
  n <- tcmDeplete
  return (tv, TyVar n)

addMonoBind :: Name -> Ty -> Env -> Env
...
lookupSpec :: Name -> Ty ->  TCM (Maybe Specialisation)
...
addSpecialisation :: Name -> Ty -> [Arg] -> Expr -> TCM ()
...
addResolution :: Name -> Ty -> Expr -> TCM ()
