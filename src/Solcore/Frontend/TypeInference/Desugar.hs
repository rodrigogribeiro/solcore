...
module Solcore.Frontend.TypeInference.Desugar where

import qualified Solcore.Frontend.TypeInference.AbsFun as C
import Solcore.Frontend.TypeInference.AbsFun(UIdent(..), LIdent(..), CType(..), CPred(..), QType(..), QPred(..))
import Solcore.Frontend.TypeInference.PrintFun(printTree)
...
instance Desugar CType Ty where
  desugar = desugarT
...
instance Desugar QType (Qual Ty) where
...
instance Desugar C.LIdent Name where
  desugar (LIdent s) = Name s

instance Desugar C.UIdent Name where
  desugar (UIdent s) = Name s
...
instance Desugar C.Decl I.Decl where
  ...
  desugar (C.Pragma i)         = I.Pragma (unName $ desugar i)
...
desugarT :: CType -> Ty
desugarT (CTVar i)    = TyVar (TVar $ desugar i)
desugarT (CTArr t u)  = desugarT t :-> desugarT u
desugarT (CTCon i ts) = TyCon (desugar i) (map desugarT ts)
desugarT (CTCon0 i)   = TyCon (desugar i) []

desugarP :: CPred -> Pred
desugarP (PSingle ct i)     = InCls (desugar i) (desugarT ct) []
desugarP (PMulti  ct i cts) = InCls (desugar i) (desugarT ct) (map desugarT cts)
